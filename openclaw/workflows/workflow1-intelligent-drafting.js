...
const { writeAuditLog, readUserMap } = require("./_shared");
...
function loadInput(inputArg) {
  if (!inputArg) {
    throw new Error("Provide input JSON string or path to JSON file.");
  }
  const maybePath = path.resolve(process.cwd(), inputArg);
  const raw = fs.existsSync(maybePath) ? fs.readFileSync(maybePath, "utf8") : inputArg;
  return JSON.parse(raw);
}

function inferCategory(headline, deliverables) {
...
function parseBudgetBnb(rawBudget) {
...
function splitMilestones(budgetBnb, deliverables, timeline) {
  ...
  // Deterministic split: 40/30/30 or 60/40
...
function main() {
  ...
  const brandAddr = ADDRESS_REGEX.test(input.brandAddr ?? "") ? input.brandAddr : "0x1111111111111111111111111111111111111111";
  const influencerAddr = INFLUENCER_CANDIDATES[category][0];
  ...
  const output = {
    ...
    suggestedInfluencers: INFLUENCER_CANDIDATES[category],
    confidence: {
      extraction: 0.93,
  ...
  writeAuditLog({
    ...
    userId: userMap[brandAddr.toLowerCase()] ?? "unknown_user",
    chainEventHash: "N/A",
