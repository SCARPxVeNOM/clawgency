...
function parseBudgetBNB(value: string): string {
  const parsed = Number(value);
  if (!Number.isFinite(parsed) || parsed <= 0) {
    throw new Error("budgetBNB must be a positive number string.");
  }
  return value.trim();
}

function parseBody(body: unknown): ParsedProposalEmailRequest {
  if (!body || typeof body !== "object") {
    throw new Error("Body must be a JSON object.");
  }

  const candidate = body as ProposalEmailRequest;
  ...
  if (!isNonEmptyString(candidate.ctaUrl) || !HTTP_URL_REGEX.test(candidate.ctaUrl.trim())) {
    throw new Error("ctaUrl must be a valid http(s) URL.");
  }
  ...
  return {
    ...
    budgetBNB: parseBudgetBNB(candidate.budgetBNB),
...
function createApprovalSessionId(campaignId: string) {
  ...
  return `proposal_${compactCampaignId}_${Date.now()}`;
}
...
export async function POST(request: Request) {
  ...
  const rate = checkRateLimit({
    request,
    scope: "api_campaigns_proposal_email",
    limit: Number.isFinite(rateLimitPerMin) ? rateLimitPerMin : 20,
    windowMs: 60_000
  });
  if (!rate.allowed) {
    return NextResponse.json(
      { error: "Rate limit exceeded for proposal email automation. Retry later." },
      {
        status: 429,
        headers: {
  ...
  let parsedBody: ParsedProposalEmailRequest;
  try {
  ...
  try {
    ...
    if (!brandProfile) {
      return NextResponse.json(
        { error: "Brand wallet is not registered. Register profile first to use proposal email automation." },
        { status: 400 }
      );
    }
    ...
    const message = error instanceof Error ? error.message : "Failed to process proposal email flow.";
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
